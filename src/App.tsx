...
export default function App() {
  ...
  async function openPath(path: string) {
    ...
    try {
      ...
      setConnectionsOpen(false); setSource(result); setSourceVersion(value => value + 1); setInputPath(result.path); setReveal(null); setSidebarVisible(true);
  ...
  function connectSource(next: Source | null) {
    if (next) setConnectionsOpen(false);
  ...
  function selectInContent(entry: TreeEntry) { setConnectionsOpen(false); setSelection(entry); setReveal({ ...entry }); }

  return <div className="app-shell">
    <a href="#selected-content" className="skip-link">선택한 내용으로 건너뛰기</a>
    <header className="app-header"><div className="app-brand"><span className="brand-symbol"><FileJson2 size={21} strokeWidth={1.6} /></span><strong>cc<span>/</span>jsonl-monitor</strong></div><div className="header-source"><button type="button" className="secondary-button compact" aria-expanded={!source || connectionsOpen} aria-controls="source-controls" onClick={() => setConnectionsOpen(value => !value)}><FolderOpen size={15} />연결 관리</button>{source && <span title={source.path}>{source.name}</span>}</div><div className="header-actions"><div className="local-status"><HardDrive size={14} />{isExtension ? '브라우저에서만 처리' : '내 컴퓨터에서 실행'}</div><ThemeSelect /></div></header>
    <div id="source-controls" className="source-controls" hidden={Boolean(source) && !connectionsOpen}>
