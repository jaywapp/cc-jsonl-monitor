...
export default function App() {
  ...
  async function openPath(path: string) {
    ...
    try {
      ...
      setSource(result); setInputPath(result.path); setReveal(null); setSidebarVisible(true);
  ...
  return <div className="app-shell">
    ...
    <header className="app-header"><div className="app-brand"><span className="brand-symbol"><FileJson2 size={21} strokeWidth={1.6} /></span><strong>cc<span>/</span>jsonl-monitor</strong><span className="app-section">기록 탐색</span></div><div className="local-status"><HardDrive size={14} />내 컴퓨터에서 실행</div></header>
    <form className="source-bar" onSubmit={(event) => { event.preventDefault(); void openPath(inputPath); }}>
    ...
    <div className={`workspace-layout ${sidebarVisible ? '' : 'sidebar-collapsed'}`}>
      {sidebarVisible && <TreePane source={source} selection={selection} onSelect={setSelection} reveal={reveal} />}
